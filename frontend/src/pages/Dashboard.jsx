...
import { RefreshCw, Zap, Mail, CheckCircle, AlertTriangle, BarChart3 } from 'lucide-react'
import { getStats, fetchEmails, classifyAll } from '../api'
...
export default function Dashboard() {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      setMessage(`Recuperate ${data.count} email`)
      await loadStats()
    } catch (e) {
  ...
  const handleClassifyAll = async () => {
    ...
    setMessage('')
    try {
      const { data } = await classifyAll()
      setMessage(`Classificate ${data.classified} email`)
      await loadStats()
    } catch (e) {
      setMessage(`Errore: ${e.response?.data?.detail || e.message}`)
    }
  ...
  return (
    <div>
      ...
      {message && (
        <div className="mb-6 p-3 bg-blue-50 text-blue-700 rounded-lg">{message}</div>
      )}
      ...
      <div className="mt-6 text-center">
        <Link to="/emails" className="text-blue-600 hover:underline">
          Vai alla lista email →
        </Link>
      </div>
    </div>
  )
